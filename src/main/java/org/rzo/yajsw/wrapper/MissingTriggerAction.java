...
class MissingTriggerAction implements TriggerAction
{
	...
	MissingTriggerAction(Executor executor, long period, int count, TriggerAction[] actions)
	{
		_count = count;
		_executor = executor;
		_actions = actions;
					...
					for (final TriggerAction action : _actions)
						if (action != null)
						{
							...
							_executor.execute(new Runnable()
							{
								...
								{
									// TODO add logger
									System.out.println("missing trigger executed, found # " + _counter + "triggers during check period");
									action.execute("");
								}
							});
						}
