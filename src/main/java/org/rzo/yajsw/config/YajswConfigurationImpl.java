...
public class YajswConfigurationImpl extends CompositeConfiguration implements YajswConfiguration
{
		...
		if (map != null)
			for (Iterator it = map.entrySet().iterator(); it.hasNext();)
			{
				...
				if (Platform.isWindows())
					result.addProperty(((String) entry.getKey()).toLowerCase(), entry.getValue());
			}
		return result;
	}
	...
	protected Object resolveContainerStore(String key)
	{
		...
		if (result != null && !result.equals(_interpolator.interpolate(result)))
	...
	static class MyStrLookup extends StrLookup
	{
		StrLookup	_strLookup;
		Set			_lookedupVars	= new HashSet();

		public MyStrLookup(StrLookup strLookup)
		{
			super();
			_strLookup = strLookup;
		}
		...
		public String lookup(String var)
		{
			...
			_lookedupVars.add(var);
			return _strLookup.lookup(var);
		}

		Set getLookedupVars()
		{
			return _lookedupVars;
		}
	}
	...
	public String getString(String key)
	{
		...
		if (value != null && !key.contains("account") && !key.contains("wrapper.image"))
